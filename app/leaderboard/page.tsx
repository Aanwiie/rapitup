...
export default function Leaderboard() {
  ...
  const getRankIcon = (rank: number) => {
    ...
    if (rank === 2) return <Medal className="w-5 h-5 text-orange-300" />;
    if (rank === 3) return <Medal className="w-5 h-5 text-amber-600" />;
    return <span className="font-pixel text-sm">{rank}</span>;
  };

  return (
    <main className="flex min-h-screen flex-col items-center p-4 bg-gradient-to-br from-yellow-900/50 to-orange-900/50">
      <header className="w-full max-w-4xl flex justify-between items-center mb-8 mt-4">
        <Button
          variant="ghost"
          onClick={() => router.back()}
          className="font-pixel text-xs text-yellow-400 hover:bg-yellow-400/10"
        >
          <ArrowLeft className="w-4 h-4 mr-2 text-yellow-400" />
          <span className="font-pixel text-xs">Back</span>
        </Button>
        <h1 className="text-2xl font-pixel text-yellow-400 pixel-text">Leaderboard</h1>
        <div className="w-10"></div> {/* Spacer for centering */}
      </header>

      <Card className="w-full max-w-4xl pixel-container bg-black/80 backdrop-blur-sm border-2 border-yellow-400/30">
        <CardHeader className="pb-2">
          <CardTitle className="text-xl font-pixel text-center text-yellow-400 pixel-text">
            Top Rappers
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="daily" className="w-full" onValueChange={setActiveTab}>
            <TabsList className="grid grid-cols-3 mb-4 bg-yellow-900/50 border border-yellow-400/20">
              <TabsTrigger
                value="daily"
                className="font-pixel text-xs text-yellow-400 data-[state=active]:bg-yellow-400/10 data-[state=active]:text-yellow-400"
              >
                Daily
              </TabsTrigger>
              <TabsTrigger
            ...
            {["daily", "weekly", "allTime"].map((period) => (
              <TabsContent key={period} value={period} className="space-y-4">
                <div className="grid grid-cols-12 gap-2 font-pixel text-xs text-yellow-400 mb-2 px-2">
                ...
                {leaderboardData[period as keyof typeof leaderboardData].map((entry) => (
                  <div
                    key={entry.rank}
                    className={`grid grid-cols-12 gap-2 items-center p-2 pixel-container bg-black/50 border-2 ${
                      entry.rank <= 3 ? "border-yellow-400/50" : "border-yellow-400/20"
                    } rounded-lg`}
                  >
                    <div className="col-span-1 flex justify-center">{getRankIcon(entry.rank)}</div>
                    <div className={`col-span-5 font-pixel text-sm ${getRankColor(entry.rank)}`}>
                      {entry.name}
                    </div>
                    <div className="col-span-3 font-pixel text-sm text-right text-yellow-400">
                      {entry.score.toLocaleString()}
                    </div>
                    <div className="col-span-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <span className="font-pixel text-sm text-yellow-400">{entry.level}</span>
